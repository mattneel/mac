defmodule MAC.Compiler do
      ...
      {bit_mac, vendor}, acc when bit_size(bit_mac) == 24 ->
        ...
        |> Map.update(bit_mac, vendor, fn
          sub_match when not is_binary(sub_match) ->
            IO.puts(
              "Discovered shorter key for existing entry at #{inspect(bit_mac)} for #{vendor} - dropping."
            )
  ...
  def update_sub_match_map({key_bitsize, _} = given, {bit_mac, vendor})
      when bit_size(bit_mac) < key_bitsize do
    IO.puts(
      "Discovered inconsistent bit-size at #{inspect(bit_mac)} for #{vendor}: expected #{
        key_bitsize
      } but got #{bit_size(bit_mac)} - dropping"
    )
